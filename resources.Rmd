# (PART\*) Resources {-}
```{r setup, include = FALSE}
library(cow)
```

```{r, echo=FALSE, results='asis'}
cow::borrow_chapter(
  doc_path = "spreadsheets-tutorial.Rmd",
  repo_name = "C-MOOR/c-moor-ottr-generic"
)
```

```{r, echo=FALSE, results='asis'}
cow::borrow_chapter(
  doc_path = "legends-tutorial.Rmd",
  repo_name = "C-MOOR/c-moor-ottr-generic"
)
```

# 16S miniCURE Guide

```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "An image of a slide titled: Human Gut 16S microbiome miniCURE Guide. Three pages from the guide are shown left to right with a description underneath each: 1) AnVIL Quickstart Sheets, 2) Guided code, 3) Interpreting results."}

ottrpal::include_slide("https://docs.google.com/presentation/d/1dEZtb0TgLbzhbr3HxQElwg8hENur2HMIzseg19OGd0g/edit#slide=id.g364010c1963_0_5")

```

We have a [comprehensive guide for this miniCURE](https://docs.google.com/presentation/d/1uipJ2uS2g8svKtWX-mN3MGvWYLWL9C0Pmv2nK6FjbCo/edit?usp=sharing)!

This guide is a complete packet that contains an overview of the learning objectives for ecah module, instructions for accessing modules on AnVIL, highlights of content, guided code blocks, and an overview of each dataset we have available for research projects!

Download the PDF for a digital reference and print out selected sheets for use in the classroom!



### Footnotes

**Contributions and Affiliations**

- Sayumi York, Notre Dame of Maryland University

Last Revised: August 2025